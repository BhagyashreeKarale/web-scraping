...
def task_8(urlist):
    ...
    for lang in f:
        ...
        for movie_url in urlist:
            id=movie_url[33:]
            file_name="/home/bhagyashri/Desktop/web scraping/task_8/"+id+".json"
            file_exists=os.path.exists(file_name)
            if file_exists == True:
                with open (file_name,"r") as jsonfile:
                    data=jsonfile.read()
                    p=json.loads(data)
                    if p["Director:"] == lang:
                        d[lang].append(p["Name:"])
                        if len(d[lang])>temp:
                            temp=len(d[lang])
                            bk=p["Director:"]
    return (bk+" has produced "+str(temp)+" movies.This director has produced max number of movies")  
