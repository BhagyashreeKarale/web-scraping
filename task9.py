...
def task_9(urlist):
    ...
    for movie_url in urlist:
        ...
        id=movie_url[33:]
        file_name="/home/bhagyashri/Desktop/web scraping/task_9/" +id+ ".json"
        file_exists=os.path.exists(file_name)
        if file_exists == True:
            ...
            sdiv=cast_div.find_all("div",class_="cast-item media inlineBlock")
            for i in sdiv:
                word=i.span['title']
                details['cast'].append(final(word))
            more_cast=cast_div.find_all("div",class_="cast-item media inlineBlock moreCasts hide")
            for k in more_cast:
                sword=k.span['title']
                details["cast"].append(final(sword))
            details["name"]=title
            with open(file_name,"w") as jsonfile:
